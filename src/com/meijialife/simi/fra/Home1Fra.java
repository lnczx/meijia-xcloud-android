...
//         }
    ...
    private void initCalendar(View v) {
        ...
        calendarManager = new CalendarManager(LocalDate.now(), CalendarManager.State.MONTH, LocalDate.now().minusYears(1), LocalDate.now().plusYears(1));
...
//                                UIUtils.showToast(getActivity(), "没有数据");
    ...
    public void getTotalByMonth() {
        ...
        String date = calendarManager.getHeaderText().replace("年", "").replace("月", "");
                    ...
                    if (StringUtils.isNotEmpty(t.toString())) {
                        ...
                        if (status == Constants.STATUS_SUCCESS) { // 正确
                            if(StringUtils.isNotEmpty(data)){
                                ...
                                LocalDate date = calendarManager.getSelectedDay();
                                calendarManager.selectDay(date);
//                                calendarManager = new CalendarManager(date, CalendarManager.State.MONTH, date, date.plusYears(1));
//                                calendarView.init(calendarManager, getActivity(), Home1Fra.this);
